            ...
            <div class="card-header row gutters-5">
                ...
                <div class="col-6 mb-4 ">
                    <div class="form-group mb-0">
                        <a href="{{route('all_orders.create')}}" class="btn btn-primary float-right">{{ translate('Add New Order ') }}</a>
                    </div>
                </div>
                <div class="dropdown  mb-2 mb-md-0">
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                        ...
                        <a class="dropdown-item" href="#" onclick="bulk_order_status('pending')">
                            {{ translate('Pending') }}</a>
                        <a class="dropdown-item" href="#" onclick="bulk_order_status('confirmed')">
                            {{ translate('Confirmed') }}</a>
                        <a class="dropdown-item" href="#" onclick="bulk_order_status('picked_up')">
                            {{ translate('Picked Up') }}</a>
                        <a class="dropdown-item" href="#" onclick="bulk_order_status('on_the_way')">
                            {{ translate('On The Way') }}</a>
                        <a class="dropdown-item" href="#" onclick="bulk_order_status('delivered')">
                            {{ translate('Delivered') }}</a>
                        {{-- <a class="dropdown-item" href="#" onclick="bulk_order_status('cancelled')">
                            {{ translate('Cancel') }}</a> --}}
                        <a class="dropdown-item" href="#" onclick="bulk_order_payment_status('paid')">
                            {{ translate('Paid') }}</a>
                        <a class="dropdown-item" href="#" onclick="bulk_order_payment_status('unpaid')">
                            {{ translate('Unpaid') }}</a>
                ...
                <div class="col-lg-2">
                    <div class="form-group mb-0">
                        <input type="text" class="form-control" id="search_by_phone"
                            name="search_by_phone"@isset($phone_search) value="{{ $phone_search }}" @endisset
                            ...
                            <tr>
                                ...
                                <td class="text-right">
                                    ...
                                    @can('view_order_details')
                                        ...
                                        <a class="btn btn-soft-primary btn-icon btn-circle btn-sm"
                                            href="{{ route('all_orders.edit', encrypt($order->id)) }}">
...
@section('script')
    <script type="text/javascript">
        ...
        function bulk_order_status(status) {
            var data = new FormData($('#sort_orders')[0]);
            $.ajax({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                url: "{{ url('admin/bulk-order-status/') }}" + '/' + status,
                type: 'POST',
                data: data,
                cache: false,
                contentType: false,
        ...
        function bulk_order_payment_status(status) {
            var data = new FormData($('#sort_orders')[0]);
            $.ajax({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                url: "{{ url('admin/bulk-payment-status/') }}" + '/' + status,
                type: 'POST',
                data: data,
                cache: false,
                contentType: false,
