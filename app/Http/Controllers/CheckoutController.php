...
class CheckoutController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        if ($request->payment_option != null) {
            ...
            //save purchase peice
            $lastOrderId = $request->session()->get('order_id');
            $this->updatePurchasePrice($lastOrderId);
    ...
    //handle order cost
    protected function updatePurchasePrice($lastOrderId)
    {

        // select order items
        $items = OrderDetail::where('order_id', $lastOrderId)->get();

        $orderCost = 0;
        foreach ($items as $item) {
            //select cost from product table

            $product          = Product::where('id', $item->product_id)->first();
            $cost             = $product->purchase_price;
            $productTotalCost = $product->purchase_price * $item->quantity;
            $orderCost       += $productTotalCost;
            OrderDetail::where('id', $item->id)
                ->update([
                    'cost' => $cost,
        ...
        // update order
        $orderInfo = Order::where('id', $lastOrderId)->select('id', 'grand_total')->first();
        $profit    = $orderInfo->grand_total - $orderCost;
        Order::where('id', $lastOrderId)->update([
            'total_cost' => $orderCost,
            'profit' => $profit
        ]);
    }//end of updatePurchasePrice
    ...
    public function store_shipping_info(Request $request)
    {
        ...
        if($request->email){
            ...
            $email = $randomString.'@dalstore.me';
        }
        ...
        if($request->is_guest){
            if(User::where('email', $data['email'])->first() != null){
                ...
                $affected = DB::table('users')->where('id', $user->id)->update(['phone' => $phone]);
