...
class ReportController extends Controller
{
    ...
    public function profits(Request $request)
    {
        ...
        foreach ($orders as $order){
            $totalShippingCost += $order->orderDetails->sum('shipping_cost');
        }
        $totalPrice = $orders->sum('grand_total') - $totalShippingCost ;
        $totalCost  = $orders->sum('total_cost');
        $profits    = $totalPrice - $totalCost ;
        //dd(($totalShippingCost));

        return view('backend.reports.profits', compact('orders', 'date', 'totalShippingCost', 'totalPrice', 'totalCost', 'profits'));
    }//end of profit
}
